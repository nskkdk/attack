...
def clone():
    ...
    try:
        ...
        pass
    try:
        r = requests.get("https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5&timeout=5000&country=all&ssl=all&anonymity=all")
        f.write(r.content)
        f.close()
    except:
        f.close()
...
def main():
    ...
    global multiple
    global mode
    mode = str(input("chọn chế độ (get/head):"))
    if mode == "":
        mode = "get"
    else:
        mode = str(mode)
    ...
    ip = str(input("ip/hots:"))
    prevent()
    if ip == "":
        print("Wrong Input! Please Try Again!")
        exit()
    page = str(input("giói (default=/):"))
    if page == "":
        page = "/"
